...
while True:
    ...
    try:
        ...
        logger.warning(
            f'{date.day}, {date.month}, {date.year}   {date.hour}:{date.minute}:{date.second}  Решаем уравнение. Введены числа {a}, {b}, {c}')
        # запишем коэф.
        break
    except:
        print("\033[34m{}".format('Допустимы только числа') + "\033[0m".format('\n'))
        logger.error(
            f'{date.day}, {date.month}, {date.year}   {date.hour}:{date.minute}:{date.second} Допустимы только числа')
...
if diskr > 0:
    ...
    print(f"x1 = {x1},  x2 = {x2}")
    logger.info(
        f'{date.day}, {date.month}, {date.year}   {date.hour}:{date.minute}:{date.second}   первый корень - {x1}, второй корень - {x2}')
    # запишем решения
    ...
    print(f"x1 = {x1}")
    logger.info(
        f'{date.day}, {date.month}, {date.year}   {date.hour}:{date.minute}:{date.second}    корень уравнения - {x1}')
    # запишем решения

else:
    print(f'Действительных корней нет, дискриминант = {diskr}')
    logger.warning(
        f'{date.day}, {date.month}, {date.year}   {date.hour}:{date.minute}:{date.second}    Действительных корней нет')
    # запишем решения

print("\033[36m{}".format('посчитаем площадь прямоугольника со сторонами x и y') + "\033[0m".format('\n'))
while True:
    logger.info(
        f'{date.day}, {date.month}, {date.year}   {date.hour}:{date.minute}:{date.second}    Считаем площадь прямоугольника')
    # обозначим действия
